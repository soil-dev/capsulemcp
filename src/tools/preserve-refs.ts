 ...
 * `update_opportunity`, and `update_party` to defeat the asymmetric PUT semantic
 * documented in NOTES-ON-CAPSULE-API.md §27.
 *
 * Capsule's PUT on `/kases/:id`, `/opportunities/:id`, and `/parties/:id`
 * can clear ID-bearing ref fields (`team`, `stage`) omitted from the body
 ...
 *
 * These update tools used to inline this read; this helper centralises
 ...
 * at `path` — typically `/kases/:id`, `/opportunities/:id`, or `/parties/:id`. Returns
