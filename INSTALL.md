...
- [Node.js 20 or newer](https://nodejs.org/) (`node --version` to check)
...
```json
{
  "mcpServers": {
    "capsule": {
      "command": "npx",
      "args": ["-y", "github:arapov/capsulemcp#v0.3.3"],
      "env": {
...
```sh
claude mcp add capsule --env CAPSULE_API_TOKEN=<your token> -- npx -y github:arapov/capsulemcp#v0.3.3
```
...
claude mcp add capsule -- npx -y github:arapov/capsulemcp#v0.3.3
```
...
1. **Bump the version in your config**: change `#v0.3.3` to a newer tag and restart.
