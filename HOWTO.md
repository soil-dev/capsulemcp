...
605 tests, all mocked — no Capsule API calls happen, no token needed. The suite has three layers:
...
- [ ] **README "N tools / N read-only" counts** still match — bumping a tool count without bumping these numbers silently drifts. The source of truth is `tests/tool-annotations.test.ts`, which asserts the live catalog size (`result.tools.length`, currently 89) and the read-only subset (`readOnly === 50`); run `npm test` and reconcile the README/CHANGELOG numbers with those assertions. (A raw `grep` over `src/server.ts` undercounts — registrations span `registerTool` / `registerBatchTool` / `registerToolTask` / `server.tool`, often multi-line.)
