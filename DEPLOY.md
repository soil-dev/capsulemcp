...
| `CAPSULE_MCP_LOG_VERBOSE` | optional | Set to `1` (or `true` / `yes` / `on`) to emit structured single-line JSON events to stderr for cache operations (`cache.hit`, `cache.miss`, `cache.invalidate`, `cache.evict`). Cloud Run's logging agent parses these into `jsonPayload` fields, so they're queryable retroactively via `gcloud logging read 'jsonPayload.event="cache.hit"'`. Off by default — flip on for a few hours when you want to measure hit rate or diagnose unexpected misses, then flip off. See `OPTIMIZATIONS.md` for recipes. Note: `batch.complete` events are emitted regardless of this flag (low volume; uniformly useful) |
